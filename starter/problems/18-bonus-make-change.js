...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
  ...
  if (target >= coins[0]) {
      target = target - coins[0];
      changes.push(coins[0]);
  } else {
      coins.shift();
  }

  const remaining = greedyMakeChange(target, coins);
  changes = changes.concat(remaining);
  return changes;
}

function makeBetterChange(target, coins) {
  if (target === 0) return [];
  ...
  for (let i = 0; i < coins.length; i++) {
      if (target >= coins[i]) {
          ...
          const nextChange = makeBetterChange(remainder, coins);

          if (nextChange !== null) {
              const currentChange = [coins[i]].concat(nextChange);
